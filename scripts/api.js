...
//         const imageFileName = `${card.setCode}-${formattedNumber}.jpg`;
//         return {
...
//         Image: `resources/imgs/${imageFileName}`,
//         Cost: card.cost ?? "",
...
//         Lore: card.lore ?? 0,
//         Rarity: card.rarity || "",
//         Unique_ID: `${card.setCode}-${formattedNumber}`,
...
export function fetchCardsData() {
  ...
  return fetch(apiUrl)
    .then(response => response.json())
    .then(data => {
      // data ya es el array de cartas
      cardsData = data.map(card => {
        const formattedNumber = String(card.Card_Num).padStart(3, "0");
        const imageFileName = `${card.Set_ID}-${formattedNumber}.jpg`;
        return {
          ...card,
          Image: `resources/imgs/${imageFileName}`
        };
